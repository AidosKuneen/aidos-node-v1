...
public class StorageApprovers extends AbstractStorage {
	...
	private static final String APPROVERS_FILE_NAME = Configuration.booling(DefaultConfSettings.TESTNET)
			? "approvers.store.testnet" : "approvers.store";
	...
	public long approveePointer(final byte[] hash) {
		synchronized (Storage.class) {
			...
			for (int depth = 2; depth < Transaction.HASH_SIZE; depth++) {
				...
				((ByteBuffer) approversChunks[(int) (pointer >> 27)].position((int) (pointer & (CHUNK_SIZE - 1))))
						.get(mainBuffer);
				...
				while (true) {
					...
					if (offset == CELL_SIZE - Long.BYTES) {
						...
						if (nextCellPointer == 0) {
							...
							((ByteBuffer) approversChunks[(int) (nextCellPointer >> 27)]
	...
	private void appendToApprovers() {
		...
		((ByteBuffer) approversChunks[(int) (approversNextPointer >> 27)]
				.position((int) (approversNextPointer & (CHUNK_SIZE - 1)))).put(mainBuffer);
		...
		for (int depth = 2; depth < Transaction.HASH_SIZE; depth++) {
			...
			if (mainBuffer[Transaction.TYPE_OFFSET] == GROUP) {
				...
				if ((pointer = value(mainBuffer, (hash[depth] + 128) << 3)) == 0) {
					...
					((ByteBuffer) approversChunks[(int) (prevPointer >> 27)]
				...
				while (true) {
					...
					if (offset == CELL_SIZE - Long.BYTES) {
						...
						if (nextCellPointer == 0) {
							...
							((ByteBuffer) approversChunks[(int) (pointer >> 27)]
