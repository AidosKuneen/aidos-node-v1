...
public class StorageBundle extends AbstractStorage {
	...
	private static final String BUNDLES_FILE_NAME = "bundles.store.testnet";
	...
	public long bundlePointer(final byte[] hash) {
		synchronized (Storage.class) {
			...
			for (int depth = 2; depth < Transaction.BUNDLE_SIZE; depth++) {
				...
				((ByteBuffer) bundlesChunks[(int) (pointer >> 27)].position((int) (pointer & (CHUNK_SIZE - 1))))
						.get(mainBuffer);
				...
				while (true) {
					...
					if (offset == CELL_SIZE - Long.BYTES) {
						...
						if (nextCellPointer == 0) {
							...
							((ByteBuffer) bundlesChunks[(int) (nextCellPointer >> 27)]
									.position((int) (nextCellPointer & (CHUNK_SIZE - 1)))).get(mainBuffer);
							offset = -Long.BYTES;
						}
					} else {
						break;
	...
	public void updateBundle(final long transactionPointer, final Transaction transaction) {
		{
			long pointer = ((transaction.bundle[0] + 128) + ((transaction.bundle[1] + 128) << 8)) << 11,
					prevPointer = 0;
			for (int depth = 2; depth < Transaction.BUNDLE_SIZE; depth++) {
				...
				if (mainBuffer[Transaction.TYPE_OFFSET] == GROUP) {
					...
					if ((pointer = value(mainBuffer, (transaction.bundle[depth] + 128) << 3)) == 0) {
						...
						((ByteBuffer) bundlesChunks[(int) (prevPointer >> 27)]
					...
					for (int i = depth; i < Transaction.BUNDLE_SIZE; i++) {
						...
						if (mainBuffer[Transaction.HASH_OFFSET + i] != transaction.bundle[i]) {
							...
							for (int j = depth; j < i; j++) {
								emptyMainBuffer();
								setValue(mainBuffer, (transaction.bundle[j] + 128) << 3,
										bundlesNextPointer + CELL_SIZE);
								appendToBundles();
							}
						...
						while (true) {
							...
							if (offset == CELL_SIZE - Long.BYTES) {
								...
								if (nextCellPointer == 0) {
									...
									((ByteBuffer) bundlesChunks[(int) (pointer >> 27)]
									...
									((ByteBuffer) bundlesChunks[(int) (pointer >> 27)]
											.position((int) (pointer & (CHUNK_SIZE - 1)))).get(mainBuffer);
									offset = -Long.BYTES;
								}
	...
	private void appendToBundles() {
		...
		((ByteBuffer) bundlesChunks[(int) (bundlesNextPointer >> 27)]
				.position((int) (bundlesNextPointer & (CHUNK_SIZE - 1)))).put(mainBuffer);
