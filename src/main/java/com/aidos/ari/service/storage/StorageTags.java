...
public class StorageTags extends AbstractStorage {
	...
	public long tagPointer(final byte[] hash) {
		synchronized (Storage.class) {
			...
			for (int depth = 2; depth < Transaction.TAG_SIZE; depth++) {
				...
				((ByteBuffer) tagsChunks[(int) (pointer >> 27)].position((int) (pointer & (CHUNK_SIZE - 1))))
						.get(mainBuffer);
				...
				while (true) {
					...
					if (offset == CELL_SIZE - Long.BYTES) {
						...
						if (nextCellPointer == 0) {
							...
							((ByteBuffer) tagsChunks[(int) (nextCellPointer >> 27)]
	...
	private void appendToTags() {
		...
		((ByteBuffer) tagsChunks[(int) (tagsNextPointer >> 27)].position((int) (tagsNextPointer & (CHUNK_SIZE - 1))))
				.put(mainBuffer);
				...
				for (int depth = 2; depth < Transaction.TAG_SIZE; depth++) {
					...
					if (mainBuffer[Transaction.TYPE_OFFSET] == GROUP) {
						...
						if ((pointer = value(mainBuffer, (transaction.tag[depth] + 128) << 3)) == 0) {
							...
							((ByteBuffer) tagsChunks[(int) (prevPointer >> 27)]
							...
							while (true) {
								...
								if (offset == CELL_SIZE - Long.BYTES) {
									...
									if (nextCellPointer == 0) {
										...
										((ByteBuffer) tagsChunks[(int) (pointer >> 27)]
